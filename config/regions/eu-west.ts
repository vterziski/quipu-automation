...
const config: RegionConfig = {
  name: 'eu-west',
  webBaseUrl: (process.env.EU_WEST_WEB_URL ?? 'https://demo.firefly-iii.org').replace(/\/$/, ''),
  apiBaseUrl: (process.env.EU_WEST_API_URL ?? 'https://demo.firefly-iii.org/api/v1').replace(/\/$/, ''),
  defaultUser: {
